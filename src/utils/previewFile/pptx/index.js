...
const renderPptx = (pptx, resultElement, thumbElement) => {
  ...
  var wrapperInnerHTML = `<style>${pptxStyle}</style>`;
  let isDone = false;
  ...
  return new Promise((resolve, reject) => {
    const processMessage = msg => {
      ...
      switch (msg.type) {
        ...
        case 'slideSize':
          break;
        case 'globalCSS':
          wrapperInnerHTML += `<style>${msg.data}</style>`;
          break;
        case 'Done':
          isDone = true;
    ...
    */
    ...
    // const resize = () => {
    //   const $section = $wrapper.querySelectorAll("section");
    //   // const slidesWidth = Math.max(
    //   //   ...Array.from($section).map(s => s.offsetWidth)
    //   // );
    //   const wrapperWidth = $section[0].offsetWidth;
