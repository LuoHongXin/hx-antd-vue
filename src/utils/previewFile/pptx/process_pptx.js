...
*/
  ...
  async function genTextBody(textBodyNode, slideLayoutSpNode, slideMasterSpNode, type, warpObj) {
    ...
    if (textBodyNode['a:p'].constructor === Array) {
      ...
      text +=
        "<div class='slide-prgrph " + getHorizontalAlign(pNode, slideLayoutSpNode, slideMasterSpNode, type, slideMasterTextStyles) + "'>";
      text += await genBuChar(pNode, slideLayoutSpNode, slideMasterSpNode, type, warpObj);
      if (rNode === undefined) {
        // without r
        text += genSpanElement(pNode, slideLayoutSpNode, slideMasterSpNode, type, warpObj);
...
*/
  ...
  function getSolidFill(node) {
    ...
    if (node['a:srgbClr'] !== undefined) {
      ...
      const sat =
        Number(defBultColorVals['sat'].indexOf('%') !== -1 ? defBultColorVals['sat'].split('%').shift() : defBultColorVals['sat']) / 100;
      const lum =
        Number(defBultColorVals['lum'].indexOf('%') !== -1 ? defBultColorVals['lum'].split('%').shift() : defBultColorVals['lum']) / 100;
