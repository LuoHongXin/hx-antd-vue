<template>
  <div>
    <funTitleDetail title="自定义图标" detail="可以针对不同的节点定制图标。"></funTitleDetail>
    <!-- 组件展示 -->
    <com-show>
      <div class="y-custom-tree">
        <y-tree showLine :tree-data="treeData" show-icon default-expand-all :default-selected-keys="['0-0-0']">
          <a-icon slot="switcherIcon" type="caret-down" />
          <a-icon slot="user" type="user" />
          <template slot="custom" slot-scope="item">
            <span class="y-custom-tree-node">
              <y-tooltip placement="topLeft" :title="item.title">
                <span class="y-custom-tree-title">{{ item.title }}</span>
              </y-tooltip>

              <y-dropdown :trigger="['click']" placement="bottomLeft">
